...
class ApiKeyService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      // Fallback to environment variables (Vercel, local .env, or process.env)
      this.keys.openai = import.meta.env.VITE_OPENAI_API_KEY || process.env.VITE_OPENAI_API_KEY || '';
      this.keys.googleMaps = import.meta.env.VITE_GOOGLE_MAPS_API_KEY || process.env.VITE_GOOGLE_MAPS_API_KEY || '';
      
      this.initialized = true;
      console.log('Fallback: API keys loaded from environment variables');
    }
  }
